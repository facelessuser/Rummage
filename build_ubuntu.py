...
def parse_settings(args, obj):
    ...
    obj.python_bin_exe = sys.executable
...
def parse_options(args, obj):
    ...
    print(obj.python_bin)

    # Construct build params for build processs
    if not err:
        obj.params = (
            (["python", obj.pyinstaller_script, '-F'] if _PLATFORM != "linux" else [obj.python_bin_exe, obj.pyinstaller_script, '-F']) +
            ...
            ['--hidden-import=gobject'] +
            ['--hidden-import=glib'] +
            ['--hidden-import=glib._glib'] +
            ['--hidden-import=glib.option'] +
            ['--hidden-import=gobject.constants'] +
            ['--hidden-import=gobject._gobject'] +
            ['--hidden-import=gobject.propertyhelper'] +
            ['--hidden-import=gtk'] +
            ['--hidden-import=gtk._gtk'] +
            ['--name=%s' % args.name] +
            ['-y', obj.script]
        )
    return err
